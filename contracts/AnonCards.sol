...
contract AnonCardsURI {
    AnonCards anoncardscontract;

    constructor(AnonCards _anoncardscontract) {
      anoncardscontract = _anoncardscontract;
    }
  ...
  {
    string memory question = anoncardscontract.question(tokenId);
    string memory answer = anoncardscontract.answer(tokenId);
    string memory replier = anoncardscontract.getHandle(anoncardscontract.replierProfileId(tokenId));
...
contract AnonCards is ERC721Royalty, ERC721Enumerable, Ownable {
  uint tokenCounter;
  bool public paused;
  IProfileNFT ProfileNFT;

  AnonCardsURI anoncardsuri;
  ...
  constructor(IProfileNFT _ProfileNFT) ERC721("AnonCards0", "ANON") {
    ProfileNFT = _ProfileNFT;
  }

  function setAnoncardsUri(AnonCardsURI _uri) public onlyOwner {
    anoncardsuri = _uri;
  }
    ...
    returns (string memory)
  {
    ...
    return anoncardsuri.tokenURI(tokenId);
  }
